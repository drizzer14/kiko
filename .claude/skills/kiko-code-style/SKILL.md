...
canonical example is the `Money` value object (see `kiko-domain`):
...
database column identifiers already fixed by `kiko-domain`.
...
the domain's own glyph map (see `kiko-domain`'s "Entity glyphs" rule).
