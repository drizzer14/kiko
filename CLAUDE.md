...
- `npm run check:all` — the fast + medium tier checks, in order:
  lint, dup, knip, deps, security, secrets, overrides, typecheck.
...
session's changed source files; knip, deps, typecheck project-wide)
runs on `Stop` and `SubagentStop`, and stays silent when no source
file changed. `tsc --noEmit` is project-wide by nature — a type error
surfaces in the file that CONSUMES a changed type, not only in the
file that was edited — so it never takes a changed-file argument. Both wrappers exit `2` on failure and print the structured
