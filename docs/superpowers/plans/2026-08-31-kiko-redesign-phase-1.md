# Kiko Redesign Phase 1 — Design System + Navigation — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Give Kiko a native-iOS shell — iOS-matched dark tokens, glass surfaces, SF Symbols, and a native bottom-tab navigation with no Home title and no back button — while keeping every existing screen reachable and every test green.
