...
- Base branch is `drizzer14/pff-ux-round` at `e7cc1fa` (worktree today: `/Users/drizzer14/orca/workspaces/pff-ios/pff-ux-round`). Do **not** branch from `main` — `main` has zero icon PNGs and `CFBundleDisplayName = PFF`.
...
- `app.json` `name` stays `PFF` — `index.js` registers the root component under it and `ios/PFF/AppDelegate.swift:27` loads `withModuleName: "PFF"`. Only `displayName` changes.
...
- App Privacy answer is **"Data Not Collected"**; it must stay consistent with `ios/PFF/PrivacyInfo.xcprivacy` (`NSPrivacyCollectedDataTypes` empty, `NSPrivacyTracking = false`). No analytics or crash reporting may be added.
...
|---|---|---|
| `ios/PFF/Info.plist` | Modify | Add `ITSAppUsesNonExemptEncryption=false`; delete `NSLocationWhenInUseUsageDescription`; delete `UISupportedInterfaceOrientations~ipad` (dead once iPhone-only). `CFBundleDisplayName = Кіко` is already there from `pff-ux-round`. |
| `ios/PFF/uk.lproj/InfoPlist.strings` | Create | Ukrainian-locale override for `CFBundleDisplayName`. |
| `ios/PFF.xcodeproj/project.pbxproj` | Modify | `IPHONEOS_DEPLOYMENT_TARGET` 15.1→16.0 in all four `XCBuildConfiguration` blocks (lines 268, 300, 369, 444); `TARGETED_DEVICE_FAMILY` `"1,2"`→`"1"` in the two target blocks (lines 284, 315); register `uk` in `knownRegions` (line 142–145); add a `PBXVariantGroup` + `PBXFileReference` + `PBXBuildFile` + group child + Resources-phase entry for `InfoPlist.strings`. |
| `app.json` | Modify | `displayName` `PFF`→`Кіко`. |
| `ios/Podfile` | Read-only check | Confirm `platform :ios, "16.0"` (line 8) is unchanged. |
| `ios/PFF/Images.xcassets/AppIcon.appiconset/` | Precondition check only | Three PNGs + `Contents.json` must already be present from `pff-ux-round`. |
...
### Task A1: Branch from `pff-ux-round` and verify the carried-forward state

**Files:**
- Read: `ios/PFF/Info.plist`, `ios/PFF/Images.xcassets/AppIcon.appiconset/Contents.json`
...
ls -1 ios/PFF/Images.xcassets/AppIcon.appiconset/
```
...
```bash
plutil -extract CFBundleDisplayName raw ios/PFF/Info.plist
```

Expected: `Кіко`. If it prints `PFF`, STOP for the same reason.
...
**Files:**
- Modify: `ios/PFF/Info.plist:25-37`
...
```bash
plutil -lint ios/PFF/Info.plist
plutil -extract ITSAppUsesNonExemptEncryption raw ios/PFF/Info.plist
plutil -extract NSLocationWhenInUseUsageDescription raw ios/PFF/Info.plist; echo "exit=$?"
```

Expected, in order:

```
ios/PFF/Info.plist: OK
false
ios/PFF/Info.plist: Could not extract value, error: No value at that key path or invalid key path: NSLocationWhenInUseUsageDescription
exit=1
```

- [ ] **Step 4: Hand off** — `git diff ios/PFF/Info.plist` should show exactly +2 / −2 lines. No commit.
...
**Files:**
- Modify: `ios/PFF.xcodeproj/project.pbxproj:268,300,369,444`
...
```bash
grep -c 'IPHONEOS_DEPLOYMENT_TARGET = 16.0;' ios/PFF.xcodeproj/project.pbxproj
grep -c 'IPHONEOS_DEPLOYMENT_TARGET = 15.1;' ios/PFF.xcodeproj/project.pbxproj
plutil -lint ios/PFF.xcodeproj/project.pbxproj
xcodebuild -project ios/PFF.xcodeproj -target PFF -configuration Release -showBuildSettings 2>/dev/null | grep -E '^\s+IPHONEOS_DEPLOYMENT_TARGET ='
xcodebuild -project ios/PFF.xcodeproj -target PFF -configuration Debug -showBuildSettings 2>/dev/null | grep -E '^\s+IPHONEOS_DEPLOYMENT_TARGET ='
```
...
4
0
ios/PFF.xcodeproj/project.pbxproj: OK
    IPHONEOS_DEPLOYMENT_TARGET = 16.0
    IPHONEOS_DEPLOYMENT_TARGET = 16.0
```
...
- [ ] **Step 4: Hand off** — `git diff --stat ios/PFF.xcodeproj/project.pbxproj` shows 4 insertions, 4 deletions. No commit.
...
**Files:**
- Modify: `ios/PFF.xcodeproj/project.pbxproj:284,315`
- Modify: `ios/PFF/Info.plist` (the `UISupportedInterfaceOrientations~ipad` array, currently lines 50–56 before Task A2's edits shift them)
...
```bash
grep -c 'TARGETED_DEVICE_FAMILY = "1";' ios/PFF.xcodeproj/project.pbxproj
grep -c 'TARGETED_DEVICE_FAMILY = "1,2";' ios/PFF.xcodeproj/project.pbxproj
plutil -lint ios/PFF.xcodeproj/project.pbxproj
xcodebuild -project ios/PFF.xcodeproj -target PFF -configuration Release -showBuildSettings 2>/dev/null | grep -E '^\s+TARGETED_DEVICE_FAMILY ='
plutil -lint ios/PFF/Info.plist
plutil -extract 'UISupportedInterfaceOrientations~ipad' raw ios/PFF/Info.plist; echo "exit=$?"
plutil -extract UISupportedInterfaceOrientations json -o - ios/PFF/Info.plist
```
...
ios/PFF.xcodeproj/project.pbxproj: OK
    TARGETED_DEVICE_FAMILY = 1
ios/PFF/Info.plist: OK
ios/PFF/Info.plist: Could not extract value, error: No value at that key path or invalid key path: UISupportedInterfaceOrientations~ipad
exit=1
...
**Files:**
- Modify: `app.json:3`
- Read-only: `index.js`, `ios/PFF/AppDelegate.swift:27`

**Interfaces:**
- Produces: `app.json` = `{ "name": "PFF", "displayName": "Кіко" }`. `name` is consumed by `index.js` (`AppRegistry.registerComponent(appName, ...)`) and must match `AppDelegate.swift`'s `withModuleName: "PFF"`; `displayName` is metadata the RN CLI reads and must match `CFBundleDisplayName`.
...
grep -n 'withModuleName' ios/PFF/AppDelegate.swift
npm run check:lint
```
...
```
{"name":"PFF","displayName":"Кіко"}
27:      withModuleName: "PFF",
```
...
**Files:**
- Create: `ios/PFF/uk.lproj/InfoPlist.strings`
- Modify: `ios/PFF.xcodeproj/project.pbxproj` — `PBXBuildFile` section (line 9–15), `PBXFileReference` section (17–28), `PBXGroup` `PFF` children (44–50), `knownRegions` (142–145), `PBXResourcesBuildPhase` files (160–164), and a new `PBXVariantGroup` section between `/* End PBXSourcesBuildPhase section */` (line 253) and `/* Begin XCBuildConfiguration section */` (line 255).

**Interfaces:**
- Consumes: the display-name string `Кіко` (Global Constraints).
- Produces: `PFF.app/uk.lproj/InfoPlist.strings` in the built bundle, so iOS shows `Кіко` under a Ukrainian locale from the localized table rather than only from the base `CFBundleDisplayName`. Three new pbxproj object IDs, fixed here so every later reference matches:
...
Create `ios/PFF/uk.lproj/InfoPlist.strings` with exactly:
...
```bash
plutil -lint ios/PFF/uk.lproj/InfoPlist.strings
plutil -convert json -o - ios/PFF/uk.lproj/InfoPlist.strings; echo
```

Expected:

```
ios/PFF/uk.lproj/InfoPlist.strings: OK
{"CFBundleDisplayName":"Кіко"}
```

- [ ] **Step 3: Register the build file**

In `/* Begin PBXBuildFile section */`, add this line directly after the `0C80B921A6F3F58F76C31292 /* libPods-PFF.a in Frameworks */` line (IDs in this section are kept in ascending order):
...
In `/* Begin PBXFileReference section */`, add this line directly after the `81AB9BB72411601600AC10FF /* LaunchScreen.storyboard */` line. The `PFF` group has no `path` of its own, so — like `path = PFF/Info.plist` — the path is relative to the project directory:

```
		A590DCB08CD54D13B7197879 /* uk */ = {isa = PBXFileReference; fileEncoding = 4; lastKnownFileType = text.plist.strings; name = uk; path = PFF/uk.lproj/InfoPlist.strings; sourceTree = "<group>"; };
```

- [ ] **Step 5: Add the variant group to the `PFF` group**

In the `13B07FAE1A68108700A75B9A /* PFF */` group, change:
...
```bash
plutil -lint ios/PFF.xcodeproj/project.pbxproj
xcodebuild -list -project ios/PFF.xcodeproj 2>/dev/null | grep -A1 'Targets:'
grep -c '053E6C3DC9C9417BACEDBE28' ios/PFF.xcodeproj/project.pbxproj
grep -c 'A590DCB08CD54D13B7197879' ios/PFF.xcodeproj/project.pbxproj
grep -c '124900AE1E7A4C17A724F221' ios/PFF.xcodeproj/project.pbxproj
grep -n 'knownRegions' -A4 ios/PFF.xcodeproj/project.pbxproj | grep -c 'uk,'
```
...
- Produces: a green `npm run check:all`; a Release `.app` at `ios/build/Build/Products/Release-iphonesimulator/PFF.app` whose `Info.plist` and `uk.lproj` prove A2–A6 landed; the privacy-policy text that Task B8 hosts.
...
```bash
xcodebuild -workspace ios/PFF.xcworkspace -scheme PFF -configuration Release \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro Max' \
  -derivedDataPath ios/build build 2>&1 | tail -3
```

Expected: `** BUILD SUCCEEDED **`. `ios/build/` is gitignored (`build/` in `.gitignore`). Release is the archive configuration in `PFF.xcscheme` (`ArchiveAction buildConfiguration = "Release"`), so this is the closest local rehearsal of Task B5.

- [ ] **Step 3: Inspect the built bundle**

```bash
APP=ios/build/Build/Products/Release-iphonesimulator/PFF.app
...
false
Кіко
PFF.app/Info.plist: Could not extract value, error: No value at that key path or invalid key path: NSLocationWhenInUseUsageDescription
exit=1
Кіко
ios/build/Build/Products/Release-iphonesimulator/PFF.app/AppIcon60x60@2x.png
ios/build/Build/Products/Release-iphonesimulator/PFF.app/Assets.car
```
...
xcrun simctl install booted ios/build/Build/Products/Release-iphonesimulator/PFF.app
...
```
 M app.json
 M ios/PFF.xcodeproj/project.pbxproj
 M ios/PFF/Info.plist
?? docs/store/
?? ios/PFF/uk.lproj/
```
...
- [ ] **Step 2 (verify):** In the worktree, `plutil -extract ITSAppUsesNonExemptEncryption raw ios/PFF/Info.plist` prints `false` and `git status --short` is clean.
...
- [ ] **Step 2 (conditional edit):** Compare the paid team's Team ID (Task B1 Step 4) with `DEVELOPMENT_TEAM = M52858LNYL` in `ios/PFF.xcodeproj/project.pbxproj` (lines 265 and 298, target Debug and Release). **If they are identical, do nothing.** If they differ (this happens when a formerly free Personal Team is superseded by a new paid team), change both lines to the new ID, e.g. `DEVELOPMENT_TEAM = <new Team ID>;`, then `plutil -lint ios/PFF.xcodeproj/project.pbxproj` → `OK`. This is the one Group B step that may touch a repo file; it goes through the same review-before-commit rule.
- [ ] **Step 3:** Open `ios/PFF.xcworkspace` in Xcode → target `PFF` → Signing & Capabilities: **Automatically manage signing** checked; Team = the paid team; Bundle Identifier shows `com.dmytro.pff`. Xcode registers the App ID `com.dmytro.pff` in Certificates, Identifiers & Profiles and creates an Apple Development certificate + profile here; the **Apple Distribution** certificate and App Store profile are created automatically at the first Archive → Distribute (Task B5).
...
- [ ] **Step 2 (Xcode route — primary):** open `ios/PFF.xcworkspace` → scheme `PFF` → destination **Any iOS Device (arm64)** → Product → **Archive**. The scheme's Archive action already uses the Release configuration. `ios/.xcode.env.local` supplies `NODE_BINARY` to the "Bundle React Native code and images" phase.
...
- [ ] **Step 2–3 (CLI alternative, same result):**
  ...
  ```bash
  xcodebuild -workspace ios/PFF.xcworkspace -scheme PFF -configuration Release \
    -destination 'generic/platform=iOS' -archivePath ios/build/PFF.xcarchive archive
  ```
  ...
  ```bash
  xcodebuild -exportArchive -archivePath ios/build/PFF.xcarchive \
...
- Branch note (land on `pff-ux-round`, do not redo icon/display name) → A1 (verified as preconditions).
...
1. **Launch screen still says "PFF".** `ios/PFF/LaunchScreen.storyboard:19` has a label with `text="PFF"` and line 24 a label `text="Powered by React Native"`. The first thing a reviewer sees on cold start will be "PFF", not "Кіко". Concrete fix if approved: change `text="PFF"` to `text="Кіко"` and delete the `Powered by React Native` label element (id `MN2-I3-ftu`) plus its constraints. Not in the spec, so not scheduled here.
