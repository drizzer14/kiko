# Plan: PFF agent harness plugin (nine agents, ponytail isolated)
...
- **Spec:** `docs/superpowers/specs/2026-08-30-pff-scaffold-and-harness-design.md` (Sections 6, 7, and 8).
...
- The plugin is local to the repo at `harness/pff-harness/`.
...
- Adds `harness/pff-harness/.claude-plugin/plugin.json`.
- Adds empty `harness/pff-harness/agents/`, `skills/`, `commands/`, `hooks/`.
...
- [ ] Create the directories, including the marketplace manifest directory:
  ...
  mkdir -p harness/.claude-plugin harness/pff-harness/.claude-plugin harness/pff-harness/agents \
    harness/pff-harness/skills harness/pff-harness/commands harness/pff-harness/hooks
  ```
- [ ] Write `harness/pff-harness/.claude-plugin/plugin.json`:
  ```json
  {
    "name": "pff-harness",
    "version": "0.1.0",
    "description": "PFF agent harness: nine role agents, isolated ponytail review, and tiered quality hooks.",
    "author": { "name": "PFF" },
...
- [ ] Both manifests parse as JSON:
  ```bash
  node -e "JSON.parse(require('fs').readFileSync('harness/pff-harness/.claude-plugin/plugin.json','utf8'));console.log('plugin ok')"
  node -e "JSON.parse(require('fs').readFileSync('harness/.claude-plugin/marketplace.json','utf8'));console.log('marketplace ok')"
  ```

**Commit:**

- [ ] `git add -A && git commit -m "feat(plugin): add pff-harness plugin skeleton"`

---

## Task 3 — The nine agent files

Create one file per agent under `harness/pff-harness/agents/`. Each file states its role, its boundaries, its recorded effort (a metadata comment), and its superpowers links. Group the commits: core coding agents first, review and test agents next, support agents last.
...
- Adds `harness/pff-harness/commands/ponytail-review.md`.
...
- [ ] Confirm the source version:
  ...
  ```
- [ ] Write `harness/pff-harness/commands/ponytail-review.md`:
  ```markdown
  ---
...
- [ ] Confirm no ponytail hooks exist and the STE style is intact for every other agent and the main session:
  ```bash
  grep -rn "ponytail" harness/pff-harness/hooks/ 2>/dev/null   # expect: no output
...
- Adds `harness/pff-harness/skills/harness-workflow/SKILL.md`.
- Adds `harness/pff-harness/skills/design-system/SKILL.md`.
- Adds `harness/pff-harness/skills/retrospect/SKILL.md`.
- Adds `harness/pff-harness/skills/ops/SKILL.md`.
...
- [ ] Write `skills/harness-workflow/SKILL.md`:
  ...
  description: Use when coordinating PFF work. States the delegation rule and points to the superpowers process skills.
  ---
...
- [ ] Write `skills/design-system/SKILL.md`:
  ...
  Keep one source of truth for PFF styles.
...
- [ ] Write `skills/ops/SKILL.md`:
  ...
  ---
  name: ops
  description: Use when running PFF builds, installs, pods, or the harness checks.
  ---
...
- Adds `harness/pff-harness/hooks/hooks.json`.
- Edits `.claude/settings.json` (removes the duplicate hook entries).

**Steps:**

- [ ] Write `harness/pff-harness/hooks/hooks.json` using the confirmed exit-2-to-stderr contract:
  ```json
  {
    "hooks": {
      "PostToolUse": [
...
- [ ] Register the local plugin for this project. Add it as a local marketplace and install it, or add it to the project plugin config:
  ...
  claude plugin install pff-harness@harness
  ```
...
- [ ] `git add -A && git commit -m "chore(plugin): register pff-harness for the project"`
...
- [ ] The plugin at `harness/pff-harness/` loads with all nine agents and the four project skills.
