# Plan: PFF app scaffold (bare React Native, iOS only)
...
- **Spec:** `docs/superpowers/specs/2026-08-30-pff-scaffold-and-harness-design.md` (Section 3 only).
...
- [ ] Create the temporary directory outside the repo:
  ```bash
  TMP="$(mktemp -d)/pff-init"
  mkdir -p "$TMP"
  ```
- [ ] Init the bare app into the temporary directory:
  ```bash
  npx @react-native-community/cli@latest init PFF --directory "$TMP/PFF" --skip-git-init --pm npm
  ```
- [ ] Confirm the app generated in the temporary directory:
  ```bash
  ls "$TMP/PFF/package.json" "$TMP/PFF/index.js" "$TMP/PFF/ios"
  ```
- [ ] Move the app files into the repo root. Use rsync to protect `.git`, `.claude`, and `docs/`:
  ...
  rsync -a --exclude='.git' --exclude='.claude' --exclude='docs' "$TMP/PFF/" ./
  ```
...
- [ ] Commit the scaffold:
  ```bash
  git add -A
  git commit -m "feat: scaffold bare React Native app (PFF)"
  ```
...
- [ ] Build a Metro bundle:
  ```bash
  npx react-native bundle --platform ios --entry-file index.js --bundle-output /tmp/pff-main.jsbundle --dev false
  ```
