...
**Root cause:** `react-native-bottom-tabs` derives its tint from the navigation theme `primary` (accent blue) but has no explicit, stable active/inactive scheme, so the user sees "random color scheme flashes" when interacting with the bar.

**User decision (locked):** keep the accent blue as the **active** tab color. Do not change the active color to white. The task is to make the scheme deterministic and stop the flash — set explicit `tabBarActiveTintColor = theme.colors.accent` and `tabBarInactiveTintColor = theme.colors.textSecondary`, and remove any implicit source that causes the color to change unexpectedly on press.

- [ ] **Step 1:** Read `react-native-bottom-tabs` current options via the `context7` MCP (confirm the exact option names for active/inactive tint, and any ripple/press-effect option for this version before coding).
- [ ] **Step 2:** Investigate the actual flash source on the device/simulator: whether it is the press ripple, the tab-bar background, or the theme-derived tint. Fix that root cause — do not only paper over it with the tint props.
- [ ] **Step 3:** Add `screenOptions` to the `Tabs.Navigator` setting `tabBarActiveTintColor = theme.colors.accent` and `tabBarInactiveTintColor = theme.colors.textSecondary` from theme tokens; disable any unwanted press ripple/highlight if that is the flash source.
- [ ] **Step 4:** Run `npm test -- navigation` (or the app-render smoke test). Expected: PASS.
- [ ] **Step 5:** `npm run check:lint`.

**Manual check:** the active tab is accent blue and stays that way; pressing and switching tabs no longer flashes an unexpected color scheme.
