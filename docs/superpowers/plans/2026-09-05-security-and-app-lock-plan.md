# Security Hardening + Biometric App Lock Implementation Plan (kiko-rename corrected)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Encrypt the on-device SQLite database with SQLCipher behind a device-only Keychain key (migrating the existing plaintext `kiko.db` once, and scrubbing every plaintext file), harden the Monobank token's Keychain policy, pin `api.monobank.ua`, exclude the database from backups, mechanize the no-`console` rule, and add a Face ID / passcode `LockGate` with a settings toggle, a foreground grace period, and a native app-switcher snapshot cover.

**Architecture:** `src/db/client.ts` stops opening the database synchronously at module load and instead exposes a lazy connection proxy plus an async `initDatabase()` that `MigrationsGate` awaits before `runMigrations()`. A new `src/db/encrypted-database.ts` owns the SQLCipher open and the one-time `sqlcipher_export` of the live plaintext `kiko.db` into `kiko-encrypted.db`, reusing the existing `migrateLegacyDatabase()` to canonicalize any pre-rename `pff.db` first, then deleting **both** plaintext files. A new `src/auth/` module wraps `@sbaiahmed1/react-native-biometrics`, holds the `useAppLock` hook (cold-launch lock + `AppState` grace math), and renders the `LockGate` that wraps the navigator inside `MigrationsGate`. Native changes (`AppDelegate.swift` privacy overlay + backup exclusion, `Info.plist` pins + Face ID string) carry explicit manual device-verification steps.

**Tech Stack:** React Native 0.87.1 (bare, Hermes, new arch), TypeScript, `@op-engineering/op-sqlite` (SQLCipher build via `package.json` `"op-sqlite": { "sqlcipher": true }`), `drizzle-orm` + `drizzle-kit`, `react-native-keychain`, new dependency `@sbaiahmed1/react-native-biometrics`, `fnts` (`either`), `ts-pattern`, `react-native-unistyles` 3, Jest + `@testing-library/react-native`, Biome, Swift (AppDelegate).

**Spec:** `docs/superpowers/specs/2026-09-04-security-and-app-lock-design.md` (source of truth). Research inputs `docs/research/2026-09-04-security-pass.md` and `docs/research/2026-09-04-biometric-app-lock.md`. This plan supersedes the earlier `docs/superpowers/plans/2026-09-04-security-and-app-lock.md`, which was written before the pff→kiko on-device migration shipped and is stale on database/Keychain naming and migration numbering.

## Corrections applied vs the stale 2026-09-04 plan

Read this section first; it is the entire reason this plan exists. Every downstream task already bakes these in.

1. **The live on-device plaintext database is `kiko.db`, not `pff.db`.** The pff→kiko rename shipped an on-device data migration: `src/db/migrate-legacy-db.ts` already `VACUUM INTO`s the old `pff.db` into `kiko.db` at module load and deliberately **leaves `pff.db` in place as a plaintext backup**. So the encryption migration's plaintext *source* is `kiko.db`, and `pff.db` is treated only as a legacy backup that must be **deleted** after the encrypted copy exists (a plaintext balances file must not survive on disk once encryption ships).
2. **Reuse `migrateLegacyDatabase()`; do not reimplement the rename.** The encryption module calls it once to canonicalize any surviving pre-rename `pff.db` into `kiko.db`, then exports from `kiko.db`. `migrate-legacy-db.ts` and its test are unchanged; its importer moves from `client.ts` to `encrypted-database.ts` (still imported, so Knip/depcheck stay green).
3. **Encrypted file name is `kiko-encrypted.db`** (the design doc's `pff-encrypted.db`, kiko-renamed). The DB-key Keychain service is **`kiko.db.key`** (the design doc's `pff.db.key`, kiko-renamed) — a brand-new key, so no Keychain migration is needed.
4. **The Monobank token service is already `kiko.monobank.token`.** Task 2 adds `accessible: WHEN_UNLOCKED_THIS_DEVICE_ONLY` to `saveToken` **and** to the `migrateLegacyToken` re-save in `src/monobank/token.ts` (line 47), so a device upgrading from the legacy `pff.monobank.token` service lands a hardened item.
5. **The lock-settings Drizzle migration is `0009`, not `0007`.** The crypto-sync branch already carries migrations through `0008` (journal idx 8). `drizzle-kit generate` will emit `0009_add_lock_settings` diffed against `meta/0008_snapshot.json`.
6. **`NSFaceIDUsageDescription` already exists in `ios/Kiko/Info.plist`** (added for the crypto Binance sync) with Binance-credential-specific wording. Task 9 **updates the string** to cover app unlock; it does **not** re-add the key.
7. **`MigrationsGate` already chains `migrateLegacyToken`.** Task 6 keeps that call — the new flow is `initDatabase()` → `runMigrations()` → `migrateLegacyToken()`. Do not drop the token migration.
8. **AppDelegate backup exclusion lists all three DB base names** — `kiko-encrypted.db`, `kiko.db`, `pff.db` — so any transient plaintext residue is also excluded while it exists.

## Global Constraints

- Work in the existing worktree `/Users/drizzer14/orca/workspaces/pff-ios/crypto-btc-sync` on branch `drizzer14/crypto-btc-sync`. Do not push. **Hold commits for user review** — one commit per task, conventional-commit subjects (`feat(db): …`, `feat(auth): …`, `chore(harness): …`, `build(native): …`). The developer may be editing `src/` concurrently; rebase/coordinate before committing overlapping files.
...
- `npm run check:all` must be green at every task's commit checkpoint (`check:lint`, `check:dup`, `check:knip`, `check:deps`, `check:security`, `check:secrets`, `check:overrides`). `npm run check:deep` (Stryker mutation ≥ 60 + osv-scanner) runs once before declaring the feature done (Task 15). The known `image-size` / `decode-uri-component` CVEs are accepted debt per `CLAUDE.md` — do not suppress them.
- `.npmrc` `min-release-age=7`: pin the new dependency to an exact version string confirmed ≥7 days old at implementation time (`npm view @sbaiahmed1/react-native-biometrics time`); if the intended version is younger, either pick an older published version or justify a `min-release-age-exclude` entry with the same category of reason as the existing `Kiko`/`react-native` entries. Its only peer beyond `react`/`react-native` must not pull in an Expo runtime (`ls node_modules/expo` → absent).
...
- Copy for the user-facing strings in this plan is final: the `NSFaceIDUsageDescription` wording in Task 9, `"Unlock Kiko"` (biometric prompt title), `"App Lock"` (settings row), `"Lock again after"` (grace label), grace labels `Immediately` / `30 sec` / `1 min` / `5 min`.
...
| `src/monobank/token.ts` (+ `token.test.ts`) | modify | `accessible: WHEN_UNLOCKED_THIS_DEVICE_ONLY` on `saveToken` **and** the `migrateLegacyToken` re-save. |
| `src/db/keys/db-key.ts` (+ `db-key.test.ts`) | create | Keychain read/store of the DB key (service `kiko.db.key`), CSPRNG key generation, SQLCipher raw-key formatting. |
| `src/db/encrypted-database.ts` (+ `.test.ts`) | create | `openEncryptedDatabase()`: SQLCipher guard, key resolution, one-time `kiko.db`→`kiko-encrypted.db` export (reusing `migrateLegacyDatabase`), plaintext scrub. |
| `package.json` | modify | `"op-sqlite": { "sqlcipher": true }`; new dependency. |
| `src/db/client.ts` (+ `client.test.ts`) | modify | Lazy `rawDatabase` proxy, `initDatabase()`, proxy-based `wrapClientForDrizzle`; drop the module-load `migrateLegacyDatabase()` call. |
| `src/db/migrations.gate.tsx` (+ new `migrations.gate.test.tsx`) | modify | `initDatabase()` → `runMigrations()` → `migrateLegacyToken()`. |
...
| `ios/Kiko/Info.plist` | modify | `NSPinnedDomains` for `api.monobank.ua`; **update** `NSFaceIDUsageDescription` wording. |
| `docs/security/README.md` | create | Pin rotation runbook + accepted-risk register. |
| `jest/setup.js` | modify | Global mock for `@sbaiahmed1/react-native-biometrics` (+ keychain fallback stub if needed). |
...
| `drizzle/migrations/0009_add_lock_settings.sql`, `meta/0009_snapshot.json`, `meta/_journal.json`, `migrations.js` | generate | Drizzle migration for the two columns. |
...
**Left unchanged (deliberately):** `src/db/migrate-legacy-db.ts` and its test — reused, not rewritten. `src/repositories/rates.repo.test.ts` and `src/db/migrate-legacy-db.test.ts` reference it and are unaffected.

**Spec items handled as verification only (no code change):** `monobank-token-field.component.tsx` prefill (Task 15 — the token has no `accessControl`, so its eager `readToken()` is a silent double *read*, not a prompt), `knip.json` / `.depcheckrc.json` (Task 9 verifies the new dependency is seen through `src/auth/biometrics.ts`; no exception is added unless a resolver false-positive is actually observed).

**Design deviations, decided here:** (1) `getOrCreateDbKey()` is split into `readDbKey` / `generateDbKey` / `storeDbKey` because the key must be persisted *after* the export succeeds. (2) The encrypted file is `kiko-encrypted.db` (op-sqlite has no rename). (3) `useAppLock` returns `{ isReady, isLocked, unlock }` — `isReady` lets `LockGate` avoid flashing real data before `settings.lockEnabled` loads. (4) `LockGate` lives in `src/auth/lock-gate/` (component-folder convention). (5) `Screen` cannot be used by `LockGate` (its `useBottomTabBarHeight()` throws outside the tab navigator), so the gate composes `SafeAreaView` + `Box` directly. (6) The encryption migration reuses `migrateLegacyDatabase()` and deletes both `kiko.db` and `pff.db` after a successful export.
...
**Files:**
- Modify: `biome.json` (`linter.rules.suspicious`, currently `{ "noExplicitAny": "error" }`)
- Modify: `docs/harness/review-to-biome-inventory.md` (append to the "Key findings" bullet list)
- Modify: `.env` (header comment)
- Modify: `CLAUDE.md` ("Dependency hygiene" section)

**Interfaces:**
- Consumes: nothing.
- Produces: `npm run check:lint` fails on any `console.*` call in Biome-scanned files.

- [ ] **Step 1: Confirm there is no `console.*` in `src` today**

Run: `grep -rn "console\." src/`
Expected: no output (verified: `src/` is console-free, so the rule lands green).
...
- [ ] **Step 3: Prove the rule bites, then confirm the repo is clean**

Create a scratch file `src/tmp-no-console.ts`:
...
- [ ] **Step 4: Record the rule in the inventory**
...
- [ ] **Step 5: Document the `.env` public-only rule**

Replace the leading comment of `.env` with (adapt to whatever header the file currently opens with — keep any real keys below it untouched):
...
- [ ] **Step 6: Run the fast checks and commit**
...
Note: `.env` is already tracked and modified in this worktree; only replace the comment header, do not alter existing values.
...
**Files:**
- Modify: `src/monobank/token.ts` (`saveToken`, line 13; and the `migrateLegacyToken` re-save, line 47)
...
- Produces: `saveToken(token: string): Promise<void>` unchanged signature; the stored item now carries `accessible: WHEN_UNLOCKED_THIS_DEVICE_ONLY` under service `'kiko.monobank.token'`. `migrateLegacyToken` re-saves the migrated token with the same policy. `readToken` / `clearToken` unchanged.

- [ ] **Step 1: Write the failing test**

Add to `src/monobank/token.test.ts` a stateful `react-native-keychain` mock (if not already present) that records the last `setGenericPassword` options, then these assertions. Keep the existing save/read/clear/migrate tests passing.
...
jest.mock('react-native-keychain', () => {
  const stores = new Map<string, { username: string; password: string }>();
  return {
    ACCESSIBLE: { WHEN_UNLOCKED_THIS_DEVICE_ONLY: 'AccessibleWhenUnlockedThisDeviceOnly' },
    setGenericPassword: jest.fn(async (username: string, password: string, options?: { service?: string }) => {
      stores.set(options?.service ?? '', { username, password });
      return true;
    }),
    getGenericPassword: jest.fn(async (options?: { service?: string }) => stores.get(options?.service ?? '') ?? false),
    resetGenericPassword: jest.fn(async (options?: { service?: string }) => {
      stores.delete(options?.service ?? '');
      return true;
    }),
  };
});
...
describe('monobank token hardening', () => {
  ...
  it('hardens the token when migrating it from the legacy service', async () => {
    await Keychain.setGenericPassword('monobank', 'legacy-token', { service: 'pff.monobank.token' });
  ...
  it('round-trips a saved token and clears it', async () => {
    await saveToken('secret-token');
...
Expected: FAIL — the `accessible` assertions fail because both writes currently pass `{ service }` only.

- [ ] **Step 3: Implement**

In `src/monobank/token.ts`, add a shared options constant and apply it to both writes. Replace `saveToken` (lines 12-14) and the migration re-save (line 47):
...
And in `migrateLegacyToken`, change the re-save line:

```ts
  await Keychain.setGenericPassword(legacy.username, legacy.password, HARDENED);
```

(If `Keychain.SetOptions` is not exported by the installed typings, type `HARDENED` as `{ service: string; accessible: (typeof Keychain.ACCESSIBLE)['WHEN_UNLOCKED_THIS_DEVICE_ONLY'] }` instead — verify at implementation.)

- [ ] **Step 4: Run the tests to verify they pass**

Run: `npx jest src/monobank/token.test.ts src/screens/use-auto-sync.test.ts`
Expected: PASS (auto-sync mocks `readToken`, so it is unaffected — this run proves it).
...
**Interfaces:**
- Consumes: `react-native-keychain` (`getGenericPassword`, `setGenericPassword`, `ACCESSIBLE`), `@op-engineering/op-sqlite` `open({ name, location: ':memory:' })` → `DB` with `executeSync(sql, params): { rows: Array<Record<string, unknown>> }` and `close(): void`.
...
Design note (why not `crypto.getRandomValues`): Hermes ships no WebCrypto. SQLite's `randomblob()` is a ChaCha20 stream keyed from the OS CSPRNG (`/dev/urandom` via the unix VFS) — CSPRNG-grade and already in the app. `Math.random` is banned for secrets by `rules/semgrep-mobile.yml` (`kiko-insecure-random-for-secrets`). If review prefers the platform CSPRNG directly, the drop-in alternative is `react-native-get-random-values` (`SecRandomCopyBytes`); that is a new dependency and needs user approval.
...
git commit -m "feat(db): Keychain-backed SQLCipher key module (kiko.db.key)"
```

---

### Task 4: SQLCipher build + `openEncryptedDatabase()` with the one-time `kiko.db`→encrypted migration

This is the highest-risk task in the plan: it moves every existing user's data and then deletes the plaintext files. The unit tests pin the crash-safe ordering (export → persist key → delete plaintext); the manual step proves real data survives on a device.
...
**Interfaces:**
- Consumes: Task 3 `readDbKey`, `generateDbKey`, `storeDbKey`, `toSQLCipherRawKey`; `src/db/migrate-legacy-db.ts` `migrateLegacyDatabase(): DB` (existing, reused unchanged); op-sqlite `open(options: { name; location?; encryptionKey?; failOnCreate? }): DB`, `isSQLCipher(): boolean`, `DB.execute(sql, params?): Promise<QueryResult>`, `DB.executeSync(sql, params?): { rows }`, `DB.delete(): void`, `DB.close(): void`, `DB.getDbPath(): string`; `fnts/either` `eitherSync`, `isLeft`, `bifold`.
- Produces:
  ```ts
...
1. Throws if `isSQLCipher()` is false (a misconfigured native build must fail loudly, never fall back to plaintext).
2. Key present in Keychain → the encrypted DB is authoritative: delete any leftover plaintext `kiko.db` and `pff.db` (crash residue), then open `kiko-encrypted.db` with the key.
3. Key absent → generate a key → run `migrateLegacyDatabase()` (idempotent) to canonicalize any pre-rename `pff.db` into the live plaintext `kiko.db` and return its connection. If it has user tables (upgrade): discard any stale partial `kiko-encrypted.db`, `ATTACH DATABASE <path> AS encrypted KEY <rawKey>` + `SELECT sqlcipher_export('encrypted')` + `DETACH DATABASE encrypted` on the plaintext connection → `storeDbKey` → close plaintext → delete **both** `kiko.db` and `pff.db`. If it has no user tables (fresh install): close it → `storeDbKey` → delete both plaintext files → open encrypted.
...
Why reuse `migrateLegacyDatabase` (Correction 1/2): the rename migration already made `kiko.db` the live plaintext file and left `pff.db` as a backup. Calling it here gives one canonical plaintext source and stays consistent with the shipped rename; the encryption layer's only additions are the SQLCipher export and the deletion of both plaintext files. SQLCipher-built op-sqlite reads an unencrypted database with no key set (backward compatible), so opening `kiko.db`/`pff.db` keyless under the SQLCipher build works — **verify this at implementation** on a real op-sqlite SQLCipher build.
...
Expected: pod install succeeds; op-sqlite compiles with `OP_SQLITE_USE_SQLCIPHER=1`.

Run: `grep -c "OP_SQLITE_USE_SQLCIPHER=1" "ios/Pods/Target Support Files/op-sqlite/op-sqlite.debug.xcconfig"`
Expected: `1` (if the path/flag differs on the installed op-sqlite version, confirm the SQLCipher define is present in the generated xcconfig by whatever key that version uses).

- [ ] **Step 2: Write the failing tests**

Create `src/db/encrypted-database.test.ts`. The plaintext source is provided by a mocked `migrateLegacyDatabase`; op-sqlite `open`/`isSQLCipher` and the key module are mocked.
...
describe('openEncryptedDatabase', () => {
  ...
  describe('steady state: key already in the Keychain', () => {
    ...
    it('does not create a plaintext file just to check for one', async () => {
      ...
      const plaintextOpens = mockOpen.mock.calls.filter(
        ([options]) =>
          options.name === LIVE_PLAINTEXT_DATABASE_NAME || options.name === LEGACY_PLAINTEXT_DATABASE_NAME,
      );
  ...
  describe('upgrade: no key, populated plaintext kiko.db', () => {
    beforeEach(() => {
      mockMigrateLegacyDatabase.mockImplementation(() => handleFor(LIVE_PLAINTEXT_DATABASE_NAME, 5));
      mockFiles.add(LIVE_PLAINTEXT_DATABASE_NAME);
      mockFiles.add(LEGACY_PLAINTEXT_DATABASE_NAME);
    });
...
const userTableCount = (db: DB): number => Number(db.executeSync(USER_TABLE_COUNT_SQL).rows[0]?.n ?? 0);
...
Run: `npx jest src/db/encrypted-database.test.ts src/db/keys/db-key.test.ts src/db/migrate-legacy-db.test.ts`
Expected: PASS (the legacy-db test is unchanged and confirms the reused module still behaves).
...
git commit -m "feat(db): SQLCipher build flag + encrypted open with one-time kiko.db export"
```
...
1. Install a **pre-encryption** build (current `crypto-btc-sync` HEAD, before this task), create at least one account + holding + a manual transaction, note the exact balances. Confirm the container has a plaintext `kiko.db` (`sqlite3 <container>/Library/kiko.db 'SELECT count(*) FROM accounts;'` returns a number).
...
4. Inspect the container: `xcrun simctl get_app_container booted com.dmytro.pff data` → `ls -la <container>/Library/` shows `kiko-encrypted.db` (plus `-wal`/`-shm`) and **no** `kiko.db` and **no** `pff.db`.
5. `sqlite3 <container>/Library/kiko-encrypted.db 'SELECT count(*) FROM accounts;'` from the Mac must print `Error: file is not a database` — proof the file is encrypted (stock macOS `sqlite3` has no SQLCipher).

Legacy (never ran the rename) path — optional, if a pre-rename build is available:
6. Install a pre-rename `pff` build, create data (plaintext `pff.db`), then install the feature build over it. Expected: data survives; container shows `kiko-encrypted.db` only, no `pff.db`, no `kiko.db`.

Fresh-install path:
7. Delete the app, install the feature build, launch. Expected: opens to an empty Home; `ls Library/` shows `kiko-encrypted.db` only.
8. Kill and relaunch twice. Expected: no migration error; data persists (proves the stored key reopens the file).
...
- Modify: `src/db/client.test.ts` (create if absent)

**Interfaces:**
- Consumes: Task 4 `openEncryptedDatabase(): Promise<DB>`. **Drops** the current module-load `migrateLegacyDatabase()` call (that logic now runs inside `openEncryptedDatabase` → `establishKey`).
- Produces (named exports, existing consumers unchanged):
  ```ts
  export const rawDatabase: DB;            // lazy proxy; throws 'Database is not initialized' before initDatabase() resolves
  export const initDatabase: () => Promise<void>;  // opens (+ configures) the encrypted DB; memoized in-flight, retryable after failure
  export const wrapClientForDrizzle: (client: DB) => DB & { executeRawAsync(query: string, params?: Scalar[]): Promise<Scalar[][]> };
  export const database: OPSQLiteDatabase<typeof schema>;  // built at module load over the lazy proxy
  export const write: <T>(work: (db: typeof database) => Promise<T>) => Promise<T>;  // unchanged behavior
  ```

Design note: the current `client.ts` opens synchronously at module load (`rawDatabase = migrateLegacyDatabase()`) and uses a **spread** `wrapClientForDrizzle`. The SQLCipher key is async, so the open must move behind `initDatabase()`, and `rawDatabase` becomes a `Proxy` — which has no own-enumerable properties, so `wrapClientForDrizzle` must also become a `Proxy` (a spread of the lazy handle would yield `{}`). Repository tests build queries with `database` and never execute them, and `run-migrations`/`use-live-query`/`write` call `rawDatabase.<method>` at call time — the proxy keeps every call site and mock unchanged.
...
describe('wrapClientForDrizzle', () => {
  it('unwraps executeRaw().rawRows for drizzle reads (executeRawAsync)', async () => {
    ...
    const executeRaw = jest.fn(async () => ({ rawRows, columnNames: ['id', 'balance'], rowsAffected: 0 }));
    const client = { executeRaw } as unknown as DB;

    const wrapped = wrapClientForDrizzle(client);
    const rows = await wrapped.executeRawAsync('SELECT id, balance FROM accounts WHERE k = ?', ['x']);
...
Expected: FAIL — `initDatabase` is not exported; the module-load `migrateLegacyDatabase()` also throws under the empty op-sqlite stub.
...
Run: `npx jest`
Expected: PASS. Every repository / screen / navigator test keeps its `jest.mock('@op-engineering/op-sqlite', () => ({ open: … }))` stub; none execute a query, so the lazy handle is never dereferenced. `client.ts` now transitively imports `react-native-keychain` (via `encrypted-database` → `keys/db-key`). If any suite fails at import with a keychain native-module error, add this global stub to `jest/setup.js` (per-file mocks in `token.test.ts` / `db-key.test.ts` still take precedence):
...
Note: `src/db/migrate-legacy-db.ts` is no longer imported by `client.ts`, but it is still imported by `encrypted-database.ts`, so Knip/depcheck stay green.
...
### Task 6: `MigrationsGate` initializes the database, runs migrations, then migrates the legacy token

**Files:**
- Modify: `src/db/migrations.gate.tsx` (the effect body + imports)
- Create: `src/db/migrations.gate.test.tsx`

**Interfaces:**
- Consumes: Task 5 `initDatabase(): Promise<void>`; existing `runMigrations(): Promise<void>`; existing `migrateLegacyToken(): Promise<void>` (already chained in the current gate — **keep it**).
- Produces: `MigrationsGate` (default export, props `{ children: ReactNode }`) renders children only after `initDatabase()` **then** `runMigrations()` **then** `migrateLegacyToken()` all resolve; shows `Migration error: <message>` if any rejects.
...
Expected: FAIL — the init-ordering test fails because `runMigrations` runs immediately (init is not awaited first), and the init-failure test renders `ready`.

- [ ] **Step 3: Implement**

In `src/db/migrations.gate.tsx`, add the `initDatabase` import and reorder the effect chain. Keep the existing `migrateLegacyToken` import and call.
...
Leave the rest of the file (state type, pending/error/children rendering) unchanged.

- [ ] **Step 4: Run the tests to verify they pass**

Run: `npx jest src/db/migrations.gate.test.tsx __tests__/App.test.tsx`
Expected: PASS (`App.test.tsx` mocks `MigrationsGate` as a passthrough).
...
git commit -m "feat(db): MigrationsGate awaits initDatabase before migrations and legacy-token migration"
```
...
**Files:**
- Modify: `ios/Kiko/AppDelegate.swift`

**Interfaces:**
- Consumes: DB base names `kiko-encrypted.db`, `kiko.db`, `pff.db` (mirrored as Swift literals of Task 4's constants).
...
Why native (the spec's "check op-sqlite's own API first"): op-sqlite exposes no backup-exclusion or file-attribute API (only `getDbPath`, `delete`, `moveAssetsDatabase`). Setting the attribute in the AppDelegate — already edited for the overlay — avoids a new dependency for one flag.

- [ ] **Step 1: Add the lifecycle hooks and helpers**

In `ios/Kiko/AppDelegate.swift`, inside `class AppDelegate`, add after `application(_:didFinishLaunchingWithOptions:)` (before the class's closing brace). Adapt to the RN 0.87 template's actual class/`window` accessor — verify how `window` is reached in the current file before pasting.
...
```swift
  ...
  /// added synchronously here is guaranteed to be in the snapshot. Solid black
  /// matches the app's true-black theme background.
...
1. Launch on the simulator, navigate to Home with visible balances.
2. Open the app switcher (Device → App Switcher, or swipe up and hold).
3. Expected: the Kiko card is solid black — no balances, no account names.
4. Tap the card to return. Expected: the overlay is gone immediately and the UI is interactive.
5. Pull Control Center down over the app and dismiss it. Expected: no lingering black cover.

- [ ] **Step 4: MANUAL VERIFICATION — backup exclusion**

```bash
CONTAINER=$(xcrun simctl get_app_container booted com.dmytro.pff data)
ls -la "$CONTAINER/Library/" | grep -E 'kiko|pff'
xattr -l "$CONTAINER/Library/kiko-encrypted.db"
```

Expected: the `xattr` output lists a backup-exclusion attribute (`com.apple.MobileBackup`, or the macOS `com.apple.metadata:com_apple_backup_excludeItem` the simulator may use). `-wal`/`-shm` files, if present, show the same attribute.
...
**Files:**
- Modify: `ios/Kiko/Info.plist` (`NSAppTransportSecurity` dict, lines 29-35)
...
Pin selection: pin **CA identities**, not the leaf, so a routine leaf renewal never breaks the app. The primary pin is the root CA (survives leaf and intermediate renewals); the current intermediate is the backup pin. **Recompute both at implementation time** — the values below are placeholders and MUST match what the commands print.
...
Record the root (primary) and intermediate (backup) SPKI-SHA256 base64 values. If the served chain differs from an Amazon-issued chain, use whatever root/intermediate the command prints and note it in the runbook's "current chain" table.

- [ ] **Step 2: Add the pins to `Info.plist`**

Replace the `NSAppTransportSecurity` dict (currently lines 29-35) with (substitute the two `<string>` values with the freshly computed ones from Step 1):
...
```xml
	...
	<dict>
		...
		<!-- Certificate pinning for the token-bearing host only. CA (not leaf)
		     pins: primary = root CA, backup = current intermediate. Rotation
		     runbook: docs/security/README.md. CoinGecko is deliberately
		     unpinned (public price data, no secret). -->
			...
			<dict>
				...
				<array>
					<dict>
						...
						<string>REPLACE_WITH_ROOT_SPKI_SHA256_BASE64</string>
					</dict>
					<dict>
						<key>SPKI-SHA256-BASE64</key>
						<string>REPLACE_WITH_INTERMEDIATE_SPKI_SHA256_BASE64</string>
					</dict>
				</array>
			</dict>
		</dict>
...
Expected: silent success. If gitleaks flags the base64 SPKI strings as a `generic-api-key` false positive, add a narrow `.gitleaks.toml` `regexes`/allowlist entry for the exact pin values (they are public certificate fingerprints, not secrets) with a justification, mirroring the existing bech32 example-address allowlist entry — do not path-allowlist the whole plist.
...
### Current chain (verified YYYY-MM-DD — fill in at implementation)

| Role | Subject | Expires | SPKI-SHA256 (base64) | Pinned |
|---|---|---|---|---|
| leaf | `CN=…` | … | `…` | no (renews) |
| intermediate | `…` | … | `…` | yes — backup |
| root | `…` | … | `…` | yes — primary |
...
1. Rebuild and run with a connected Monobank account. Trigger Sync. Expected: sync succeeds.
2. Negative test: change the last character of **both** `SPKI-SHA256-BASE64` strings, rebuild, trigger Sync. Expected: sync fails (TLS / `NSURLErrorServerCertificateUntrusted`). Revert both, rebuild, confirm sync succeeds again.
...
```bash
git add ios/Kiko/Info.plist docs/security/README.md .gitleaks.toml
git commit -m "build(native): pin api.monobank.ua CA identities; add security runbook and accepted-risk register"
```

(Omit `.gitleaks.toml` if Step 2 did not need it.)

---

### Task 9: Biometrics dependency + `src/auth/biometrics.ts` wrapper + Face ID string update

**Files:**
- Modify: `package.json` (dependencies), `package-lock.json`
- Modify: `ios/Podfile.lock` (via `pod install`)
- Modify: `ios/Kiko/Info.plist` (**update** the existing `NSFaceIDUsageDescription`, lines 36-37 — do NOT add a second key)
- Modify: `jest/setup.js` (global mock)
- Create: `src/auth/biometrics.ts`
- Create: `src/auth/biometrics.test.ts`

**Interfaces:**
- Consumes: `@sbaiahmed1/react-native-biometrics` named exports `isSensorAvailable(): Promise<{ available: boolean; biometryType?: string; isDeviceSecure?: boolean; error?: string; errorCode?: string }>` and `authenticateWithOptions(options): Promise<{ success: boolean; error?: string; errorCode?: string }>`. **Verify the exact export names, option keys, and iOS error-code strings against the installed version at implementation time** — the codes below are from the package's `ReactNativeBiometricsError.swift` and drive the mapping.
- Produces:
  ```ts
...
**Biometric-crash guard (memory lesson):** a `BIOMETRY_CURRENT_SET` Keychain read SIGABRT-crashes without `NSFaceIDUsageDescription`, and units cannot catch it. This app does not gate any Keychain item on biometrics, so it does not hit that class. Still, `authenticateWithOptions` (Face ID prompt) requires `NSFaceIDUsageDescription` — it is already present (Correction 6). Step 2 only refines the wording; do not remove or duplicate the key.

- [ ] **Step 1: Install the dependency and native pod**

Run: `npm view @sbaiahmed1/react-native-biometrics time` and pick an exact version ≥7 days old. Then `npm install --save-exact @sbaiahmed1/react-native-biometrics@<version>`.
Expected: `package.json` gains the pinned version; no `min-release-age` refusal; `ls node_modules/expo` → absent (the Expo peer is optional).

Run: `cd ios && pod install && cd ..`
Expected: `ios/Podfile.lock` gains the biometrics pod entry.

- [ ] **Step 2: Update the Face ID usage string (do not add a second key)**

In `ios/Kiko/Info.plist`, the key already exists (Binance-specific wording). Replace its `<string>` value (currently line 37) so it covers both app unlock and credential access:

```xml
	<key>NSFaceIDUsageDescription</key>
	<string>Kiko uses Face ID to unlock the app and your saved credentials.</string>
```

Run: `plutil -lint ios/Kiko/Info.plist`
Expected: `OK`. Confirm there is exactly ONE `NSFaceIDUsageDescription` key: `grep -c NSFaceIDUsageDescription ios/Kiko/Info.plist` → `1`.

- [ ] **Step 3: Register a global Jest mock**

Append to `jest/setup.js` (adjust the export names to the installed version if they differ):

```js
// @sbaiahmed1/react-native-biometrics is a TurboModule with no Jest binary.
// The Settings screen and LockGate reach it, both transitively from App.tsx /
// root.navigator tests, so it is mocked globally. Defaults model a Face ID
// device with a passcode; tests that exercise the wrapper's mapping override
// these with their own `jest.mock` factory.
jest.mock('@sbaiahmed1/react-native-biometrics', () => ({
  isSensorAvailable: jest.fn(async () => ({ available: true, biometryType: 'FaceID', isDeviceSecure: true })),
...
const NOT_ENROLLED_CODES = ['BIOMETRY_NOT_ENROLLED', 'FACE_ID_NOT_ENROLLED', 'TOUCH_ID_NOT_ENROLLED'] as const;
...
Run: `npm run check:all`
Expected: silent success — `knip` and `depcheck` both see the import in `src/auth/biometrics.ts`, so no `knip.json` / `.depcheckrc.json` entry is added. If either tool reports the package, diagnose the resolver issue rather than adding an ignore; add a documented ignore ONLY if the false positive is real and matches the existing native-peer pattern (e.g. `react-native-screens`).
...
**Files:**
- Modify: `src/db/use-live-query.ts`
...
Append inside the `describe('useLiveQuery', …)` block of `src/db/use-live-query.test.ts` (reuse the file's existing `fakeQuery` thenable-double pattern and its `renderHook`/`act` imports):
...
Expected: FAIL — `expect(received).toBe(true)` with `received: undefined` (`isLoading` is not returned yet).

- [ ] **Step 3: Implement**

In `src/db/use-live-query.ts`:

Change the signature's return type (currently `): { data: T[]; error?: Error } {`):

```ts
): { data: T[]; error?: Error; isLoading: boolean } {
```

Add the state right after `const [error, setError] = useState<Error | undefined>();`:

```ts
  const [isLoading, setIsLoading] = useState(true);
```

In `runQuery`, immediately after the out-of-order/unmount guard `if (!alive || myGeneration !== generation.current) { return; }` and before `const settled = first(result, toError);`, add:

```ts
      setIsLoading(false);
```

Change the final `return { data, error };` to:
...
### Task 11: Lock settings — schema columns, Drizzle migration `0009`, repository setters, grace options

**Files:**
- Create: `src/auth/lock-grace.ts`
- Create: `src/auth/lock-grace.test.ts`
- Modify: `src/db/schema.ts` (`settings` table, lines 106-112)
- Generate: `drizzle/migrations/0009_add_lock_settings.sql`, `drizzle/migrations/meta/0009_snapshot.json`, `drizzle/migrations/meta/_journal.json`, `drizzle/migrations/migrations.js`
- Modify: `src/repositories/settings.repo.ts`
- Modify: `src/repositories/settings.repo.test.ts`

**Interfaces:**
- Consumes: `write`, `database` from `src/db/client.ts`; the existing settings-repo test harness (its `write`-capture mock).
- Produces:
  ```ts
...
describe('lock grace options', () => {
  it('offers exactly the four grace periods, in ascending order', () => {
...
Add to `src/repositories/settings.repo.test.ts` (mirror the file's existing `setBaseCurrency` capture-mock pattern — reuse whatever `write`-capture helper it already imports):
...
In `src/db/schema.ts`, replace the `settings` table (lines 106-112) with:
...
export const settings = sqliteTable('settings', {
  ...
  // App lock (Face ID / passcode gate). Off by default; the grace period is how
  // long the app may sit in the background before it re-locks (0 = always).
...
- [ ] **Step 7: Generate the migration (0009, not 0007)**

Run: `npx drizzle-kit generate --name add_lock_settings`
Expected: creates `drizzle/migrations/0009_add_lock_settings.sql` and `meta/0009_snapshot.json`, appends an `idx: 9` entry to `meta/_journal.json`, and regenerates `migrations.js` with an `m0009` import + entry (diffed against `meta/0008_snapshot.json`).

Verify the SQL contains exactly two statements:
...
If drizzle-kit emits anything touching other tables, STOP and reconcile — it must diff only the two new settings columns against `0008`. Do not hand-edit the numbering: if it emits a different index (e.g. because an uncommitted migration exists in the tree), regenerate from a clean state and confirm the next free index.

Run: `npx jest src/db/run-migrations.test.ts`
Expected: PASS (the runner is bundle-agnostic; confirms nothing regressed).

- [ ] **Step 8: Add the repository setters**

In `src/repositories/settings.repo.ts`, add a `LockGraceSeconds` type import and two setters after `setLastSyncAt`, mirroring `setBaseCurrency`:
...
(Keep the two-group, shortest-first import ordering — the `../auth/lock-grace` type import joins the relative group.)

- [ ] **Step 9: Run the tests to verify they pass**

Run: `npx jest src/repositories/settings.repo.test.ts src/auth/lock-grace.test.ts`
Expected: PASS.
...
git commit -m "feat(db): lockEnabled/lockGraceSeconds settings + 0009 migration + repo setters"
```
...
- Produces:
  ...
  ```
  (The return shape is a module-private `type AppLock`; not exported because no other module names it, and Knip fails on an unused exported type.)

Behavior: `isReady` is false until the settings live query settles once. On that first settle, `locked = lockEnabled`. `isLocked = lockEnabled && locked` (turning the setting off unlocks immediately; turning it on mid-session does not lock until the next background). `AppState`: the first `inactive`/`background` stamps `Date.now()` (a later `background` after `inactive` keeps the earlier stamp); on `active`, re-lock iff `lockEnabled && shouldRelock(...)`, then clear the stamp. Transitions while an `unlock()` is in flight are ignored — the biometric sheet itself makes the app `inactive` then `active`, which would otherwise re-lock a grace-0 user in a loop. A successful `unlock()` clears the stamp and sets `locked = false`.
...
type AppLock = {
  ...
  // nor the children before then, so no real data can flash.
  isReady: boolean;
  isLocked: boolean;
...
/**
 * Pure grace-period decision: re-lock when the app was backgrounded and at least
 * `graceSeconds` have passed since. A zero grace period re-locks on every foreground.
 */
...
- Modify: `App.tsx` (import + the `MigrationsGate` subtree)
- Modify: `__tests__/App.test.tsx` (add a `LockGate` passthrough mock)

**Interfaces:**
- Consumes: Task 12 `useAppLock()`, Task 9 `AuthResult`; design-system `Box`, `Text`, `SymbolIcon`, `Button` (confirm the exact import paths/prop names for these primitives against the current design system before writing — names below follow the existing convention); `SafeAreaView` from `react-native-safe-area-context`.
...
Create `src/auth/lock-gate/lock-gate.component.tsx` (adjust primitive imports/props to the actual design system):
...
In `App.tsx`, add the import (relative group, sorted by length):
...
In `__tests__/App.test.tsx`, after the `MigrationsGate` mock, add (import `ReactNode` if not present):
...
1. Settings → App Lock on. Kill the app, relaunch. Expected: black gate with "Locked", the Face ID sheet appears automatically. Simulator: Features → Face ID → Matching Face → Home appears.
2. Relaunch, Features → Face ID → Non-matching Face twice, then choose the passcode fallback. Expected: passcode entry sheet; entering the simulator passcode unlocks.
3. Grace `Immediately`: background (Home), foreground → locked again. Grace `30 sec`: background, foreground within 30 s → not locked; wait 35 s, foreground → locked.
4. Settings → App Lock off. Background/foreground and relaunch. Expected: never locked.
5. Confirm the app-switcher card is black in every state (Task 7 overlay) and the Face ID sheet does not leave a stuck black overlay afterwards.

---

### Task 14: Settings UI — `OptionPills` primitive, `AppLockSetting` card, screen wiring

Before writing: read the current `src/design-system/components/currency-switch/currency-switch.component.tsx`, `src/screens/settings/settings.screen.tsx`, `settings-row.component.tsx`, and the `Switch`/`GlassSurface`/`Box`/`Text`/`PressableButton` primitives to confirm exact prop names and paths — the code below follows the existing conventions but the design system may have drifted since this plan was written.
...
The pill selector is extracted from `CurrencySwitch` because a second consumer now exists (project rule: shared components, no hand-rolled duplicates). `CurrencySwitch` keeps its name and props and becomes a one-line delegate.
...
Create `src/design-system/components/option-pills/option-pills.component.tsx` (match the markup/props to what `CurrencySwitch` renders today):
...
// A row of pill buttons for a small closed set of options (base currency, lock
// grace period). Only the selected pill paints a raised surface. Generic over
...
Replace `src/design-system/components/currency-switch/currency-switch.component.tsx` with a delegate (keep its existing prop types/import paths):
...
Expected: PASS — the existing settings-screen currency tests still pass through the delegate.
...
export type AppLockSettingProps = {
  lockEnabled: boolean;
  // The stored value; any integer from the DB. Highlighted only when it is one
  // of LOCK_GRACE_OPTIONS.
  lockGraceSeconds: number;
  onToggle: (enabled: boolean) => void;
  onSelectGrace: (seconds: LockGraceSeconds) => void;
};
...
Create `src/screens/settings/app-lock-setting/app-lock-setting.component.tsx` (match `SettingsRow`/`Switch`/`GlassSurface` props to the current design system):
...
- [ ] **Step 8: Wire the card into the settings screen + update the screen tests**

In `src/screens/settings/settings.screen.test.tsx`, extend the settings live-query/repo mock to carry `lockEnabled`/`lockGraceSeconds` and `setLockEnabled`/`setLockGraceSeconds` (mirror the existing `setBaseCurrency` mock), update any button-count assertion (the App Lock card adds a `switch`, not a `button`, while off; when on it adds four grace `button`s), and append:
...
In `src/screens/settings/settings.screen.tsx`, add imports (two-group, shortest-first):
...
Add handlers next to the existing currency handler:
...
Insert the card as its own sibling (blank line between sibling cards, per the JSX layout rule), reading from the same `settings` live-query row the screen already uses:
...
Run: `npm run check:all`
Expected: silent success (`check:dup` — the `CurrencySwitch` pill markup now exists once, in `OptionPills`).
...
Run: `npx jest src/screens/account-detail/monobank-token-field.component.test.tsx` (adjust the path to where the token field's test actually lives).
Expected: PASS. No code change: `readToken()` has no `accessControl`, so the eager prefill is a silent second Keychain *read* (on a screen that only mounts after `LockGate` unlocks), never a second *prompt*.
...
Expected: Stryker mutation score ≥ 60 (the `break` threshold); osv-scanner reports only the accepted advisories documented in `CLAUDE.md` (the two `image-size` and the `decode-uri-component` findings). If the mutation score dips below 60, strengthen tests in the new `src/auth/*` / `src/db/*` files (typical survivors: the `>=` boundary in `shouldRelock`, the error-code lists in `biometrics.ts`, the `lockEnabled &&` guard in `useAppLock`, the crash-safety ordering in `encrypted-database.ts`) — never lower the threshold.

- [ ] **Step 4: Device checklist (all manual steps in one pass)**

Build once (`npm run ios`, or via the ops agent) and walk:
1. Task 4 Step 7 — upgrade with data (data intact; `kiko.db` **and** `pff.db` gone; `kiko-encrypted.db` unreadable by stock `sqlite3`); fresh install; relaunch ×2.
2. Task 7 Steps 3-4 — app-switcher card is black; `xattr -l` shows the backup-exclusion attribute on `kiko-encrypted.db`.
3. Task 8 Step 4 — Monobank sync succeeds; with both pins corrupted it fails; reverted it succeeds. CoinGecko price sync unaffected.
...
5. Settings: with Face ID **not** enrolled (Features → Face ID → Enrolled unchecked), the App Lock switch stays enabled with the "No Face ID enrolled" hint; there is no simulator mode for "no passcode", so the `passcodeNotSet` branch is covered by the unit test only.

- [ ] **Step 5: Hand off**

No commit is expected from this task unless Step 3 required test strengthening (`test(auth): …` / `test(db): …`). Report to the coordinator: the task list, the manual checklist results, and the open decisions flagged in this plan (see "Open decisions" below).

---

## Self-review (against the design spec)

- **DB encryption** — Tasks 3, 4, 5, 6 (SQLCipher key, encrypted open + one-time `kiko.db` export, lazy init, gate wiring). Corrected: source is `kiko.db`, `pff.db` is a deleted-after backup, encrypted file `kiko-encrypted.db`, key service `kiko.db.key`.
- **Backup exclusion** — Task 7 (`NSURLIsExcludedFromBackupKey`, all three DB base names).
- **Token hardening** — Task 2 (`accessible` on `saveToken` and the legacy-service migration re-save; service `kiko.monobank.token`).
- **Certificate pinning + runbook** — Task 8 (`NSPinnedDomains`, `docs/security/README.md`).
- **no-console + `.env` note** — Task 1.
- **App lock (biometrics, hook, gate, settings, Face ID string, snapshot overlay)** — Tasks 9-14. Corrected: Info.plist string is *updated*, not re-added; migration is `0009`; `MigrationsGate` keeps `migrateLegacyToken`.
- **Edge cases** (`BIOMETRY_CURRENT_SET`, `PASSCODE_NOT_SET`, `BIOMETRY_LOCKOUT`, pre-existing token, stale pins, deleted DB file) — covered in Tasks 9/12/14 tests and the accepted-risk register (Task 8).
- **Out of scope** (biometric-gated token read, per-screen re-auth, jailbreak detection, CoinGecko pinning, in-app pin rotation, multi-user) — honored; jailbreak recorded as accepted risk.

## Open decisions to surface to the user

1. **Delete the `pff.db` plaintext backup during encryption.** The rename kept `pff.db` as a safety-net backup. This plan deletes it after the encrypted copy exists, because leaving a plaintext balances file on disk defeats the encryption feature. If the user wants to retain a (still-plaintext) backup for a grace period, that is a security trade-off to make explicitly.
2. **`kiko-encrypted.db` as the permanent file name.** op-sqlite has no rename, so the encrypted DB keeps a distinct name from the plaintext `kiko.db`. Acceptable, but it means a future reader must know the live DB is `kiko-encrypted.db`.
3. **DB-key entropy from SQLite `randomblob()`** vs. adding `react-native-get-random-values` (`SecRandomCopyBytes`). The former needs no new dependency; the latter is the platform CSPRNG directly. Flagged in the accepted-risk register.
4. **New dependency `@sbaiahmed1/react-native-biometrics`** — needs user sign-off (like the prior reanimated/gesture-handler/noble-hashes approvals) and a `.npmrc`-safe pinned version.
5. **Face ID usage-string wording** — Task 9 changes the shipped Binance-specific string to a general "unlock the app and your saved credentials." Confirm the wording.

## Risks

- **DB-encryption migration of existing `kiko.db` data (Task 4) is the top risk.** It moves every user's data and then deletes the plaintext source. Mitigations baked in: crash-safe ordering (export → store key → delete plaintext), stale-partial-target cleanup, key-present steady-state scrub, and mandatory on-device upgrade verification (Task 4 Step 7). Residual risk: the pathological "Keychain key survives but encrypted file deleted while plaintext survives" case loses data — documented as accepted (Task 8 register).
- **SQLCipher build correctness.** If `"op-sqlite": { "sqlcipher": true }` + `pod install` does not actually produce a SQLCipher build, `assertSQLCipherBuild()` fails closed at launch (no silent plaintext fallback). Verify the `OP_SQLITE_USE_SQLCIPHER` define lands (Task 4 Step 1) and that a keyless open of the plaintext `kiko.db` still reads under the SQLCipher build (SQLCipher backward-compat) before trusting the export.
- **Biometric-gate crash class (memory lesson).** A `BIOMETRY_CURRENT_SET` Keychain read SIGABRT-crashes without `NSFaceIDUsageDescription`, and units cannot catch it. This design gates no Keychain item on biometrics (only `accessible`), so it avoids that class entirely; the only biometric surface is `authenticateWithOptions`, which needs — and already has — the Info.plist key. Do not introduce a biometric `accessControl` on the DB key or token without re-checking this.
- **Concurrent developer edits to `src/`.** The developer is editing `src/` on this branch. Tasks touching `client.ts`, `migrations.gate.tsx`, `schema.ts`, `settings.repo.ts`, `use-live-query.ts`, `App.tsx`, and the settings screen may conflict; reconcile against the live tree before each commit, and re-verify the current file shapes (line numbers in this plan are indicative).
- **op-sqlite API surface** (`isSQLCipher`, `encryptionKey`, `executeRaw`, `failOnCreate`, `getDbPath`, `delete`, `close`) and the biometrics package's export/error-code surface must be verified against the installed versions — both are called out as implementation-time checks.
