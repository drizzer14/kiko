...
describe('HoldingCard', () => {
  ...
  it('washes the card with a 45deg gradient of its stored color on first render', async () => {
    ...
    const stops = entityGradientStops(darkTheme.colors.entityColors.violet);
    expect(getByTestId('holding-card-gradient-from').props.stopColor).toBe(stops.from);
    expect(getByTestId('holding-card-gradient-to').props.stopColor).toBe(stops.to);
  });

  it('washes the card with a gradient of the type default color when it has no stored color', async () => {
    const { getByTestId } = await render(
      <HoldingCard holding={holding({ color: null })} now={NOW} onOpen={jest.fn()} />,
    );

    // A `card` holding with no color reads the card type default (white).
    const stops = entityGradientStops(darkTheme.colors.entityColors.white);
    expect(getByTestId('holding-card-gradient-from').props.stopColor).toBe(stops.from);
    expect(getByTestId('holding-card-gradient-to').props.stopColor).toBe(stops.to);
  });
