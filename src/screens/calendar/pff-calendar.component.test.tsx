...
import PffCalendar from './pff-calendar.component';
...
describe('PffCalendar', () => {
  it('pins the grid to a fixed six-week height so months never reflow', async () => {
    const { getByTestId } = await render(<PffCalendar testID="cal" onDayPress={jest.fn()} />);
  ...
  it('forwards minDate and maxDate so out-of-range days are greyed out', async () => {
    const { getByTestId } = await render(
      <PffCalendar testID="cal" onDayPress={jest.fn()} minDate="2026-03-01" maxDate="2026-03-31" />,
    );
